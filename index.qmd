...
ggplot(df_expandido |> filter(Ano == '2023'), aes(x = subgrupo, y = remun)) +
  ...
  labs(
    title = "Distribuição de Remuneração por Subgrupo",
    x = "Subgrupo",
    y = "Remuneração"
  ) +
  theme_minimal()
...
ggplot(df_expandido |> filter(Ano == '2026'), aes(x = subgrupo, y = remun)) +
  geom_boxplot(fill = "skyblue", color = "black", alpha = 0.5) +  # Boxplot
  stat_summary(  # Adiciona a média como ponto
    fun = mean,  
    geom = "point",  
...
```{r}
ggplot(df_expandido, aes(x = remun)) +
